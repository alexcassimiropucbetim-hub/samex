...
import { Home, MapPin, Church, ListMusic, Music, Music2, Users, UserCheck, ChevronDown, FolderOpen, Briefcase, FileSignature, CalendarClock, LogOut, Settings } from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <aside className="w-64 glass h-screen flex flex-col fixed left-0 top-0 border-r border-[rgba(255,255,255,0.1)] z-50 overflow-y-auto">
        ...
        {menuGroups.map((group) => {
          ...
          return (
            <div key={group.name} className="flex flex-col">
              ...
              <div className={clsx("flex flex-col gap-1 overflow-hidden transition-all duration-300 origin-top", isOpen ? "max-h-[500px] opacity-100 scale-y-100" : "max-h-0 opacity-0 scale-y-0")}>
                {group.items.map(item => {
                  const Icon = item.icon;
                  const isActive = pathname === item.href;
                  return (
