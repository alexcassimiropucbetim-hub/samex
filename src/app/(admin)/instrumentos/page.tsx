          ...
          {instruments.length > 0 && (
            <div className="lg:hidden flex flex-col gap-4 mt-4">
              {instruments.map((instrument) => {
                const isEditing = editId === instrument.id;
                return isEditing ? (
                  <div key={instrument.id} className="bg-blue-50 rounded-xl p-4 shadow-sm border border-blue-100 flex flex-col gap-3">
                    <form action={async (formData: FormData) => {
                      "use server";
                      await updateInstrument(instrument.id, formData);
                      redirect(`/instrumentos?page=${page}&q=${q}`);
                      ...
                      <div className="flex items-center gap-2 justify-end pt-2 border-t border-blue-200/50 mt-1">
                        ...
                        <Link href={`/instrumentos?page=${page}&q=${q}`} className="text-slate-500 bg-slate-200 hover:bg-slate-300 p-2.5 rounded-xl transition-colors" title="Cancelar">
                          <X className="w-5 h-5" />
                        </Link>
                      </div>
                    </form>
                  ...
                  <div key={instrument.id} className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex flex-col gap-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="font-bold text-slate-900 uppercase text-sm">{instrument.name}</span>
                          <span className="text-xs text-slate-500 uppercase flex items-center gap-1 mt-0.5">
                            <ListMusic className="w-3 h-3" /> {instrument.category.name}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 justify-end pt-2 border-t border-slate-100">
                      <Link href={`/instrumentos?page=${page}&q=${q}&edit=${instrument.id}`} className="text-slate-500 hover:text-blue-500 p-2.5 rounded-xl hover:bg-blue-50 transition-colors">
              ...
              {totalPages > 1 && (
                <div className="flex flex-col gap-2 mt-2">
                  <div className="flex items-center justify-between">
                    {page > 1 ? (
                      <Link href={`/instrumentos?page=${page - 1}&q=${q}`} className="px-4 py-2 text-sm border border-slate-200 bg-white rounded-xl shadow-sm text-slate-600 font-medium w-full text-center hover:bg-slate-50">
                        Anterior
                      </Link>
                    ) : (
