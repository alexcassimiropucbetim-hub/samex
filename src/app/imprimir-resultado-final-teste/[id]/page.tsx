...
export default async function ImprimirResultadoFinalTestePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const approvedCandidates = test.candidates.filter(c => c.finalTestStatus === "APROVADO");
        ...
        <div className="flex items-stretch justify-between mb-8">
          ...
          <div className="flex gap-4">
            ...
            <div className="bg-[#0a2342] rounded-2xl p-4 flex flex-col justify-center items-center text-white shadow-md min-w-[120px] relative overflow-hidden">
              ...
              <div className="text-[9px] font-medium text-white/80 mt-1">Aprovados</div>
              <div className="text-4xl font-bold mt-1 leading-none">{approvedCandidates.length}</div>
            </div>
          </div>
        </div>
          ...
          <table className="w-full border-collapse text-[10px]">
            <thead>
              <tr className="bg-[#0a2342] text-white uppercase tracking-wider text-left">
                ...
                <th className="px-4 py-4 font-bold text-center">Resultado</th>
              </tr>
            </thead>
            <tbody>
              {approvedCandidates.map((candidate, idx) => (
                <tr key={candidate.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-[#f8fafd]'}>
                  ...
                  <td className="border-b border-gray-100 px-4 py-3.5 font-bold uppercase text-center text-emerald-600">
                    APROVADO
                  </td>
                </tr>
              ))}
              {approvedCandidates.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-slate-500 font-medium bg-white">
                    Nenhum candidato aprovado até o momento.
                  </td>
                </tr>
              )}
            </tbody>
