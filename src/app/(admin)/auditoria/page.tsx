...
import { Activity, ShieldCheck, User, Database } from "lucide-react";
    ...
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Histórico de alterações e atividades no sistema.</p>
        </div>
        
        <div className="w-full md:w-auto min-w-[300px]">
          <SearchInput defaultValue={q} placeholder="Buscar ação, entidade, usuário..." />
        </div>
      </div>
                  ...
                  <tr key={log.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-slate-900">
                            {log.adminUsername || log.personInCharge?.fullName || "Sistema/Desconhecido"}
                          </p>
                          {log.personInCharge && (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col">
                        ...
                        {log.entityId && <span className="text-xs text-slate-400 font-mono mt-0.5" title={log.entityId}>ID: {log.entityId.slice(0, 8)}...</span>}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-slate-500 text-xs">
                      {log.details ? (
                        <div className="max-w-xs md:max-w-md max-h-20 overflow-y-auto bg-slate-50 p-2 rounded border border-slate-100 font-mono">
          ...
          <div className="lg:hidden flex flex-col gap-4 p-4 bg-slate-50/50">
            {logs.map(log => (
              <div key={log.id} className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex flex-col gap-3">
                ...
                <div className="grid grid-cols-1 gap-2 text-xs bg-slate-50 p-2.5 rounded-lg border border-slate-100 mt-1">
                  <div>
                    <p className="text-slate-400 font-medium mb-0.5">Entidade Afetada</p>
                    <p className="text-slate-600">{log.entityType} {log.entityId ? <span className="font-mono text-[10px] ml-1">({log.entityId})</span> : ""}</p>
                  </div>
                  {log.details && (
                    <div>
                      <p className="text-slate-400 font-medium mb-0.5">Detalhes</p>
                      <pre className="text-slate-600 overflow-x-auto text-[10px] whitespace-pre-wrap">{JSON.stringify(JSON.parse(log.details), null, 2)}</pre>
                    </div>
                  )}
                </div>
              </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 bg-slate-50/50">
            ...
            <div className="flex items-center gap-2">
              {page > 1 ? (
                <Link href={`/auditoria?page=${page - 1}&q=${q}`} className="px-3 py-1.5 text-sm border border-slate-200 rounded-lg hover:bg-slate-100 text-slate-600 transition-colors bg-white">
                  Anterior
                </Link>
              ) : (
